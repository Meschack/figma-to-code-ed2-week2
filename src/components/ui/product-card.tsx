      ...
      <div className='group relative aspect-[322.67/405] min-h-[405px] overflow-hidden rounded-4xl'>
        ...
        <div className='absolute flex size-full flex-col justify-between p-5 text-sm group-hover:bg-[#000000]/20'>
          ...
          <div className='mt-auto hidden items-center gap-1 group-hover:flex'>
            ...
            <Button variant='transparent' className='font-semibold uppercase text-white hover:bg-transparent'>
              Buy now
            </Button>
          </div>
        </div>

        <Link className='absolute inset-0' href={`/product/${encodeURIComponent(product.id)}`}></Link>
      </div>

      <div className='space-y-0.5 font-semibold'>
        <h3 className='truncate text-3xl uppercase text-black'>{product.title}</h3>
